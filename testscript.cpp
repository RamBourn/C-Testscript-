...
/*生成汇编文件名称测试*/
TEST(Function1,GenerateName)
{
    //before
    string filename="test.cpp";
    string generate=asmfile(filename);
    EXPECT_EQ(0,generate.compare("test.asm"));
    //after
}

/*符号优先级测试*/
TEST(Function2,Level)
{
    //before
    string opt="#";
    int generate=level(opt);
    EXPECT_EQ(1,generate);
    //after
}


/*符号优先级测试*/
TEST(Function3,Level)
{
    //before
    string opt="+";
    int generate=level(opt);
    EXPECT_EQ(3,generate);
    //after
}

/*操作符判断测试*/
TEST(Function4,is_operator)
{
    //before
    char opt='/';
    int generate=is_operator(opt);
    EXPECT_EQ(1,generate);
    //after
}
...
/*语句结束测试*/
TEST(Function5,is_end)
{
    //before
    char opt=';';
    int generate=is_bracket(opt);
    EXPECT_EQ(1,generate);
    //after
}
...
/*单词类型判断测试*/
TEST(Function7,int_judge)
{
    //before
    string name="int";
    int generate=word_token(name);
    EXPECT_EQ(K_TYPE,generate);
    //after
}

/*关键字判断*/
TEST(Function8,main_judge)
{
    //before
    string name="main";
    int generate=word_token(name);
    EXPECT_EQ(K_KEYWORDS,generate);
    //after
}
